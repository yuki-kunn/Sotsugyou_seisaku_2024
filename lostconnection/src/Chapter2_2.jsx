...
const Chapter2_2 = () => {
  ...
  useEffect(() => {
    fetch(
      "https://script.google.com/macros/s/AKfycbxZry64-CsWMXcN2LLzDdwToep8Z0Gz-1aLC_LZoB4ZcOqSOPIKFH7BeuzDUXU7PcIZ/exec?action=chaP2_2"
    )
  ...
  useEffect(() => {
    ...
    // 背景スプライトの設定
    const background = PIXI.Sprite.from("assets/hashi.png");
  ...
  useEffect(() => {
    if (appRef.current && storyTexts.length > 0) {
      ...
      // 美咲 (右側に表示)
        ...
        if (currentStory.se === "nod") {
          const initialY = partner.y;
          ...
          ticker.add(() => {
            ...
            if (elapsed > 50) {
              partner.y += direction * 5; // 5px上下に移動
              if (partner.y > initialY + 10 || partner.y < initialY - 10) {
                direction *= -1; // 移動方向を反転
              }
              elapsed = 0;
            }
